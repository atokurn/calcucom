...
// .wrangler/tmp/pages-fLzk7p/functionsWorker-0.9055232324327822.mjs
...
__name(isoFromNow, "isoFromNow");
__name2(isoFromNow, "isoFromNow");
function nowIso() {
...
__name(valueAt, "valueAt");
__name2(valueAt, "valueAt");
function recordId(record, prefix, index) {
  return String(valueAt(record, "id", "productId") || `${prefix}-${index + 1}`);
}
__name(recordId, "recordId");
__name2(recordId, "recordId");
...
async function replaceCloudState(env, userId, state = {}) {
  const db = requireDb(env);
  const now = nowIso();
...
__name(replaceCloudState, "replaceCloudState");
__name2(replaceCloudState, "replaceCloudState");
async function getCloudState(env, userId) {
  const db = requireDb(env);
...
__name(getCloudState, "getCloudState");
__name2(getCloudState, "getCloudState");
...
__name2(getRedirectUri, "getRedirectUri");
...
__name2(buildGoogleAuthUrl, "buildGoogleAuthUrl");
...
__name(fetchGoogleUser, "fetchGoogleUser");
__name2(fetchGoogleUser, "fetchGoogleUser");
...
function sessionCookie(value, maxAge = 60 * 60 * 24 * 7) {
...
// .wrangler/tmp/bundle-eU4M8p/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-eU4M8p/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9055232324327822.js.map
